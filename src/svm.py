...
OUTPUT_SVM = r"C:\Python\THESIS\skillab_job_fetcher\output\svm_results"
...
# ---------- Train/Eval ----------
def train_eval_svm(X_text: pd.Series, y: pd.Series, labels_order: list[str], tag: str):
    ...
    out_base = Path(OUTPUT_SVM) / f"svm_{tag}"
    ...
    summary_csv = Path(OUTPUT_SVM) / "svm_summary_metrics.csv"
...
if __name__ == "__main__":
    ...
    if len(labels_multi) >= 2:
        train_eval_svm(X_multi, y_multi, labels_multi, tag="multiclass_h0_h1subtypes")
    else:
        print("[INFO] Multiclass SVM skipped (not enough classes).")

    print("\n=== SVM baselines completed ===")
    print(f"Output folder: {OUTPUT_SVM}")
